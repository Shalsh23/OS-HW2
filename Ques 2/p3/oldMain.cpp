...
pdocs_itr pd_itr;
pthread_barrier_t corpus_barrier;
int counter = 0;
...
{ 
    ...
    list<char*> thread_doc = *thread_docp;  //contains the doc that thread has to currently parse
    HASH_MAP_PARSED_DOCS thread_list_pdocs; //contains the list of parsed docs generated by this thread, for which it will now calculate tfidf
    ...
    thread_vector = v3.cal_tfidf(thread_list_pdocs,corpusOccurences);
...
int main(int argc, char *argv[]) {
    ...
    itr = docs.begin();             //iterator through docs which will allocate work to each thread when it asks for it
    list<dlist> thread_work[n];     //the array of the list in which each thread stores its doc to be parsed    
    
    pthread_mutex_init(&allParsedDocs_lock,NULL); //mutex for updating the global variable all_parsedDocs

    pthread_cond_init(&docsParsed_cv, NULL);    //cv on which main is waiting to wake up when the docs have been parsed by threads
    pthread_mutex_init(&docParsed_mutex, NULL);

    int i;
 
    pthread_cond_init(&corpus_cv, NULL);    //corpus created yet? //threads wait on this cv
    ...
    while(docsParsed_flag!=1)
        ...
        pthread_cond_wait(&docsParsed_cv,&doc_lock);


    //main wakes up and creates corpus occurence table from the parsed docs
